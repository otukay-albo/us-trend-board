...
// doliz/tiktok-creative-center-scraper（cookie必須・Creative Center実データ）
// 入力: target / cookies / hashtags_country / hashtags_period / hashtags_industry
// industry 14000000000 = Beauty & Personal Care。返却は [{ items: [{hashtagName, publishCnt, vv, rankIndex}] }]
// cookieは TIKTOK_CC_COOKIE（数日で期限切れ→要再取得）。未設定/失敗時はデモ。
const BEAUTY_INDUSTRY_ID = "14000000000";

export async function fetchHashtags() {
  const token = process.env.APIFY_TOKEN;
  const actor = process.env.APIFY_HASHTAG_ACTOR;
  const cookies = process.env.TIKTOK_CC_COOKIE;
  if (!token || !actor || !cookies) return HASHTAG_DEMO;
  ...
  try {
    const res = await fetch(
      `https://api.apify.com/v2/acts/${actor.replace("/", "~")}/run-sync-get-dataset-items?token=${token}`,
      {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          target: "trending_hashtags",
          cookies,
          hashtags_country: "US",
          hashtags_period: "7",
          hashtags_industry: BEAUTY_INDUSTRY_ID,
          hashtags_limit: 20,
        }),
        signal: AbortSignal.timeout(120_000),
      }
    ...
    // cookie切れ/障害時は items が無い → デモにフォールバック
    if (!Array.isArray(items) || items.length === 0) return HASHTAG_DEMO;
    return items.slice(0, 20).map((item) => ({
      name: item.hashtagName ?? item.hashtag_name ?? item.name ?? "—",
      sub: hashtagSub(item.publishCnt, item.vv),
    }));
  } catch {
    return HASHTAG_DEMO;
  }
