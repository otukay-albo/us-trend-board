...
export async function fetchMusic() {
  try {
    const res = await fetch(
      "https://rss.applemarketingtools.com/api/v2/us/music/most-played/15/songs.json",
    ...
    return MUSIC_DEMO.slice(0, 15);
  }
}

// ── Artists ─────────────────────────────────────────────────
// Apple RSS にアーティスト単体ランキングが無いので、人気曲100件を
// アーティスト単位で集計（チャート登場数 = バズ度）して TOP15 を作る。
...
export async function fetchBeauty() {
  ...
  if (!token || !actor) return BEAUTY_DEMO.slice(0, 15);
  ...
  try {
    ...
    if (!Array.isArray(items) || items.length === 0) return BEAUTY_DEMO.slice(0, 15);
...
export async function fetchHashtags() {
  ...
  if (!token || !actor) return HASHTAG_DEMO.slice(0, 15);
  ...
  try {
    ...
    // 障害時などで空 → デモにフォールバック
    if (!Array.isArray(items) || items.length === 0) return HASHTAG_DEMO.slice(0, 15);
    return items.slice(0, 15).map((item) => ({
    ...
    return HASHTAG_DEMO.slice(0, 15);
  }
}
